...
- 一个 daemon 自动发现并管理多个项目可用的 RuntimeInstance；RuntimeProfile 不等于在线进程。
- Event Journal 与 Read Model 共用一个 SQLite 文件，但保持逻辑分层。
- 每个 Run 使用独立受管 workspace；原始 Source 默认不由 Agent 直接写入。
- 大型 Evidence 使用 daemon 独占的本地内容寻址 Artifact Store。
- Credential Broker 位于 daemon 内，真实 Provider 凭证不进入 Agent 进程。
- AgentGrant 只允许当前认领代次调用 Loom 本地能力，不是 Provider 凭证。
...
| RuntimeInstance 离线 | 不再接受新认领；已认领 Run 按进程和 heartbeat 事实收敛 |
| prepare lease 过期 | 递增认领代次后重新认领；拒绝旧代次晚到消息 |
...
- 每个 RuntimeInstance 具有独立容量；每条 Run lineage 同时只有一个有效认领代次；

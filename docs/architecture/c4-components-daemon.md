...
C4Component
  ...
  Container_Boundary(daemon, "Loom Daemon") {
    ...
    Component(scheduler, "Scheduler and Supervisor", "Process service", "Owns runs, deadlines, cancellation and cleanup")
    ...
    Component(runtimeAdapters, "Runtime Adapter Manager", "Adapter layer", "Translates versioned Bridge messages")
  ...
  Rel(scheduler, runtimeAdapters, "Starts and controls Agent runs")
...
Domain services depend on interfaces, not Runtime or SQLite implementations. Adapter、security 和 persistence components 实现这些接口，防止产品合同被具体 Agent CLI 或 Provider SDK 反向控制。

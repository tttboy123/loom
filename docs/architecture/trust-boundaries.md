...
flowchart LR
    ...
    subgraph LoomZone["Trusted Loom Zone"]
        ...
        Workspaces[("Managed Run Workspaces")]
    ...
    Daemon -->|"claim generation + bounded dispatch"| Main
    Main -->|"orchestrated WorkItem"| Worker
    Grant -->|"scoped AgentGrant"| Main
    Grant -->|"scoped AgentGrant"| Worker
    Worker -->|"authorized events / evidence / broker request"| Daemon
    Daemon -->|"create / verify source digest"| Workspaces
    Main -->|"assigned workspace only"| Workspaces
    Worker -->|"assigned workspace only"| Workspaces
...
| AgentGrant 明文 | 当前 Run 与 claim generation 的受限 IPC 或环境 | SQLite、Bridge payload、日志、Sidecar、Provider |
| CredentialGrant 明文 | Broker 受限 IPC 或短期内存 | Agent 普通 Prompt、SQLite、日志、Sidecar |
| AgentDefinition / RuntimeProfile / RuntimeInstance | Loom State、Team Draft 目录快照 | Provider 请求正文，除非任务明确需要 |
| 原始 Source | 用户项目目录、只读 digest 输入 | Agent 可写挂载、Artifact GC |
| Run workspace | 当前 Agent process group、Daemon | 其他 Run、Sidecar、未授权 Client |
...
- Run 消息的 `claim_generation` 或 RuntimeInstance 不匹配时直接拒绝。
...
- Runtime 自动发现只产生候选 RuntimeInstance；没有客户规则和有效 AgentGrant 不能启动或调用 Loom。
